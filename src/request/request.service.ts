...
export class RequestService {
  ...
  async createRequest(userId: string, type: RequestType) {
    try {
      console.log('Creazione richiesta per utente:', userId, 'Tipo:', type);
      ...
      return await this.prisma.request.create({
        data: { userId, type, status: RequestStatus.PENDING },
      });
    } catch (error) {
